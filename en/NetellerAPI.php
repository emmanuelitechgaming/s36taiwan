    ...
    protected function makeHttpRequest($method, $path, $queryParams = array(), $headers, $requestParams = array()){
        ...
        switch (strtolower($method))
        {
            ...
            case 'post':
				...
				if (count($requestParams) > 0)
                {
					curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($requestParams, JSON_UNESCAPED_UNICODE));
				}
                break;
            case 'get':
                break;
...
class CreateCustomer extends NetellerAPI{
    ...
    public function setContactDetails($contactDetail){
    ...
    public function doRequest(){
        ...
        if(isset($this->contactDetails)){
            $requestParams["contactDetails"] = $this->contactDetails;
        }
